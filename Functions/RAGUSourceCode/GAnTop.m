function [gfp,p,AllP] = GAnTop(data,n_iter,flags)
...
RDiff(1,1) = sqrt(mean(mean(data,1).^2,2));
gfp = RDiff(1);
if ~quiet
...
for i = 2:n_iter                                                             % Here starts the randomization loop
    ...
    RDiff(i) = sqrt(mean(mean(rdata,1).^2,2));
    if ~quiet
        p = sum(RDiff > RDiff(1))/i;                                         % We compute an ad-hoc p for the impatient user
        set(h,'Name',sprintf('GANTOP: %2.0f%%    p: %0.3f',i/n_iter*100,p));    % and display it
        waitbar(i/n_iter,h);    
