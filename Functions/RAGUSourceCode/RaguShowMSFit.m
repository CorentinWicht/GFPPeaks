...
function ShowAssignment(SelectedMSClass,Indices,tbl, nBootstrapSampling)

if nargin < 4
    nBootstrapSampling = 0;
end
...
switch(Indices(1))
    case 1
        [gm,outlbl,CLabels,GLabels] = RaguGrandMeans(out.V,Group,AllC,out.DLabels1,out.DLabels2,GroupLabels ); % TK 14.3.2013
    case 2
        [gm,outlbl,CLabels,GLabels] = RaguGrandMeans(out.V,Group,out.Design(:,1),out.DLabels1,out.DLabels2,GroupLabels );

    case 3
        [gm,outlbl,CLabels,GLabels] = RaguGrandMeans(out.V,Group,out.Design(:,2),out.DLabels2,out.DLabels1,GroupLabels );
    case 4
        [gm,outlbl,CLabels,GLabels] = RaguGrandMeans(out.V,Group,out.Design,out.DLabels1,out.DLabels2,GroupLabels );
end
...
if nargin > 2
    switch tbl
        case 1
            ResTblData = on(:,:,SelectedMSClass)* out.DeltaX + out.TimeOnset;
            ttl = sprintf('Onset class %i',SelectedMSClass);
        case 2
            ResTblData = off(:,:,SelectedMSClass)* out.DeltaX + out.TimeOnset;
            ttl = sprintf('Offset class %i',SelectedMSClass);
        case 3
            ResTblData = dur(:,:,SelectedMSClass)* out.DeltaX;
            ttl = sprintf('Duration class %i',SelectedMSClass);
        case 4
            ResTblData = auc(:,:,SelectedMSClass)* out.DeltaX;
            ttl = sprintf('AUC class %i',SelectedMSClass);
        case 5
            ResTblData = cog(:,:,SelectedMSClass)* out.DeltaX + out.TimeOnset;
           ttl = sprintf('COG class %i',SelectedMSClass);
         case 6
            ResTblData = msgfp(:,:,SelectedMSClass);
            ttl = sprintf('GFP class %i',SelectedMSClass);
    end
end
...
%        end
        ...
        tmph = text(mean(xlim),ylim(2),[outlbl{g,c} infotxt],'Interpreter','none','VerticalAlignment','top','HorizontalAlignment','center');
        set(tmph,'ButtonDownFcn',{@ShowBarGraph,gcf,ResTblData,CLabels,GLabels,ttl});
...
function ShowBarGraph(obj,event,CallingFigure, tblData,clbl,glbl,ttl)
...
if isrow(tblData)
    AdjustX = true;
    tblData = vertcat(tblData,nan(size(tblData)));
end


figure();
bar(tblData,'grouped');
set(gca,'XTickLabel',glbl);

if AdjustX == true
    set(gca,'XLim',[0.6 1.4]);
    get(gca)
end
legend(clbl,'Location','southoutside','Orientation','horizontal');
title(ttl);
