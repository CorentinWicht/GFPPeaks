...
if nargout > 1
    ...
    SSRand(2,1,:) = squeeze(sum(sum(sum(SWiseMeans .^2,3),2),1));
    SSRand(1,1,:) = SSRand(2,1,:);
    
    SSRand(2,2,:)  = squeeze(sum(sum(sum(SWiseMeansF1 .^2,3),2),1));
    SSRand(1,2,:)  = SSRand(2,2,:);

    SSRand(2,3,:) = squeeze(sum(sum(sum(SWiseMeansF2 .^2,3),2),1));
    SSRand(1,3,:) = SSRand(2,3,:);

    SSRand(2,4,:) = squeeze(sum(sum(sum(SWiseMeansF1F2 .^2,3),2),1));
...
LMap = sum(DesignR .* in_s,2); % The covariance maps,1s x 1cond x channels x time
if nFlag
    disp('Dissimilarity not inplemented for continuous predictors');
end
...
LMap = sum(sum(in.*TotDesignR,1),2);
...
for l = 1:nGroupLevel
    ...
    in_s = mean(in(Group == GroupLevel(l),:,:,:),1);
    % and weight by the within factor
    LMap(l,1,:,:) = sum(DesignR .* in_s,2);
end
