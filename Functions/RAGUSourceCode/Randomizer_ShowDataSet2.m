function Randomizer_ShowDataSet2(handle)

% Copyright 2009-2018 Thomas Koenig
% distributed under the terms of the GNU AFFERO General Public License

figure(handle);
uitableNames = uitable;
out = get(handle,'UserData');
...
d.hMapTArea = subplot('Position',[0.68 0.28 0.2 0.02]);
d.hMapArea  = subplot('Position',[0.68 0.06 0.2 0.2]);
axis(hPlotArea,'off');
axis(d.hMapArea,'off');
axis(d.hMapTArea,'off');
...
if (nargin < 2) || isempty(pos)
    ...
    if verLessThan('matlab','8.0') == false
