            ...
            switch ClickType
                ...
                case 'Double' % If p < Threshold, select the significant area
                    ...
                    if y(tmpidx) <= d.Threshold && d.Averaged == 0
                        idx(1) = find(y(1:tmpidx)   > d.Threshold,1,'last' )+1;
                        idx(2) = find(y(tmpidx:end) > d.Threshold,1,'first')+ tmpidx - 2;
                    else
                        idx = [tmpidx tmpidx];
                    end
                    FaceAlpha = 0.2;
...
if (d.Averaged == 0)
    ...
    for i = 1:numel(sh)
        ...
        y = get(tanh,'YData');
        txt{i} = sprintf('%s: Exp Var = %4.2f%%, p = %5.5f',get(sh(i),'UserData'),es(idx(1)),y);
        set(get(sh(i),'Title'),'String',txt{i},'Interpreter','none','FontSize',10);
    end
...
DataToShow = mean(DataToShow(:,:,:,idx(1):idx(2)),4);
