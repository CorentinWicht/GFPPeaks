...
if vararginmatch(varargin,'Label')
    ...
    if vararginmatch(varargin,'LabelColor')
        LabBkG = varargin{vararginmatch(varargin,'LabelColor')+1};
    else
        LabBkG = [1 1 1];
    end
...
if NoExPol == 0
    ...
    axh.Units = oldUnits;
    LineWidth = nwidth / res / 25;
    ang=0:0.01:(2*pi + 0.01); 
    xp=r_max*cos(ang);
    yp=r_max*sin(ang);
    plot(xp*0.980,yp*0.980,'-k','LineWidth',LineWidth);
end
...
if NoseRadius > 0
    ...
    patch(xc,yc,ones(size(xc))-1000,[1 1 1],'LineWidth',2);
...
hold off
set(gca,'xtick',[],'ytick',[],'xticklabel',[],'yticklabel',[],'Visible','off','XLim',[-xmx-15 xmx+15],'YLim',[-ymx-15 ymx+15+NoseRadius],'DataAspectRatio',[1 1 1]);
freezeColors;
