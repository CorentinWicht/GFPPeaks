...
% Last Modified by GUIDE v2.5 16-Nov-2016 15:15:07
...
rawDir = dir(fullfile(in.LoretaSysDir,'*.raw'));
if isempty(rawDir)
...
if (isfield(in,'Max'))
    set(handles.editMax,'String',num2str(in.Max));
end

if (isfield(in,'Threshold'))
    set(handles.editThreshold,'String',num2str(in.Threshold));
end
...
fp = fopen(fullfile(in.LoretaSysDir,AnatomyFile{DefaultAnatomy}));
ud.Anatomy = reshape(fread(fp,'uint8'),[181,217,181]) / 2;
fclose(fp);

fid = fopen(fullfile(in.LoretaSysDir,'MNI-BAs-6239-voxels.csv'),'rt');
...
axis(handles.TopView,'equal');
axis(handles.SideView,'equal');
axis(handles.BackView,'equal');
...
if numel(ud.Threshold) == 1
    idx = find(abs(v) > ud.Threshold);
    v = v / ud.Max * 63;
    v = MyClip(v,63);
    res(idx) = v(idx)+192.5;
else
    for i = 1:numel(ud.Threshold)
    res(v < -ud.Threshold(i)) = 191-i;
    res(v >  ud.Threshold(i)) = 192+i;
    end
end
...
nThresholds = numel(ud.Threshold);

if(nThresholds > 1)
    cm = [(1-gray(128));ones(128,3)];
    dummy = [cool(nThresholds);zeros(2,3);hot(nThresholds)];
    cm((192 - nThresholds):(193 + nThresholds),:) = dummy;
    colormap(cm);
else
    colormap([(1-gray(128));LorBlueRed(128)]);
...
ud = get(handles.figure1,'UserData');
ud.Threshold = str2num(get(handles.editThreshold,'String'));
ShowColorScale(handles,ud);
set(handles.figure1,'UserData',ud);
...
function SelectAnatomy(MainHandle,Name)

handles = guidata(MainHandle);
ud = get(handles.figure1,'UserData');

hObject = handles.lbAnatomies;
path = get(hObject,'UserData');

fp = fopen(fullfile(path,Name));
ud.Anatomy = reshape(fread(fp,'uint8'),[181,217,181]) / 2;
fclose(fp);
...
fp = fopen(fullfile(path,fnames{idx}));
ud.Anatomy = reshape(fread(fp,'uint8'),[181,217,181]) / 2;
fclose(fp);
...
function [Images,cm] = GetAllTopViews(MainHandle)

ud = get(MainHandle,'UserData');
Images = [];
for idx = 1:21
    z = 47 + 5 * idx;
    Images = [Images GetTopImage(ud,z)];
end
...
if(nThresholds > 1)
    ...
    cm = [(1-gray(128));LorBlueRed(128)];
end



% --- Executes on button press in SaveAllTopViews.
function SaveAllTopViews_Callback(hObject, eventdata, handles)
% hObject    handle to SaveAllTopViews (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
for idx = 1:21
    z = 47 + 5 * idx;
    subplot(4,6,idx,'align');
    imagesc(GetTopImage(ud,z),[0 255]);
    axis xy equal off;
    title(sprintf('Z = %i',z -ud.ZOff));
end
...
% --- Executes on button press in AllBackViews.
function AllBackViews_Callback(hObject, eventdata, handles)
% hObject    handle to AllBackViews (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
for idx = 1:36;
    y = 20 + 5 * idx;
    subplot(4,9,idx,'align');
    imagesc(GetBackImage(ud,y),[0 255]);
    axis xy equal off;
    title(sprintf('Y = %i',y -ud.YOff));
end
...
% --- Executes on button press in AllSideViews.
function AllSideViews_Callback(hObject, eventdata, handles)
% hObject    handle to AllSideViews (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
for idx = 1:29;
    x = 16 + 5 * idx;
    subplot(4,8,idx,'align');
    imagesc(GetSideImage(ud,x),[0 255]);
    axis xy equal off;
    title(sprintf('X = %i',x -ud.XOff));
end
