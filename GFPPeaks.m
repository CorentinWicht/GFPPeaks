...
root_folder = uigetdir(p2,'Choose the path of your most upper folder containing your .EP files.');
cd(root_folder)
FileList = dir(['**/*' '.ep']);
...
PromptIndivGFP = questdlg('Would you like to use semi-automatic or manual method to identify individual GFP peaks ?', ...
	'Individual GFP peaks','Semi-automatic','Manual','Semi-automatic');
...
OutputFolder = PromptInputs{3}; mkdir([p2 '\' OutputFolder]);
if numchannels>63
...
p = uitable('Parent', f,'Data',to_display,'ColumnEdit',true(1,size(to_display,2)),'ColumnName',...
    {'Name','Lower bound (ms)','Upper bound (ms)','Elect 1','Elect 2'},'CellEditCallBack','CompList = get(gco,''Data'');');
...
uicontrol('Style', 'text', 'Position', [60 350 450 130], 'String',...
        ...
        newline '4th column = Representative electrode 1 (e.g. CPz)',...
        newline '5th column = Representative electrode 2 (e.g. Cz)',]});
% Wait for t to close until running the rest of the script
waitfor(p)
...
% Ticks = linspace(1,Epoch(2)+abs(Epoch(1)),length(XTStr));
...
% Taken from "Matlabroutinesforcartoolers" functions 
% https://sites.google.com/site/cartoolcommunity/files
Fields = fieldnames(EEGData); Pos = 1; Numbers = repmat({[]},[size(AllNames,2) 1]);

% Create a .txt file for outputs
date_name = datestr(now,'dd-mm-yy_HHMM');
fid = fopen([p2 '\' OutputFolder '\Results_' date_name '.txt'],'w');
...
for n=1:sum(~cellfun(@(x) isempty(x), CompList(:,1))) % For each Comp
    ...
    % Component
%     TEMPComp = cellfun(@(x) str2num(x),CompList(n,2:end)); % Load current Component bounds
    ...
    for k=1:length(Fields) % For each levels
        for m=1:size(EEGData.(Levels{k}),3) % For each subject
            ...
            if strcmpi(PromptIndivGFP,'semi-automatic')
                ...
                Title = strrep(Names.(Levels{k}){m},'_','-'); % Main title
                ...
                for j=1:3
                    subplot(2,3,TempPos(j)); 
                    if j==1
                        plot(mean(EEGData.(Levels{k}),3), 'Color','k'); 
                        title('All electrodes'); % axis tight;
                    else
                        TempData = mean(EEGData.(Levels{k}),3);
                        plot(TempData(:,ismember(lower({Chanlocs.labels}),lower(TempElect{j-1}))), 'Color','k'); 
                        title(TempElect{j-1}); % axis tight;
                    end
                ...
                % 2. GFP
                subplot(2,3,5); plot(TEMPGFP, 'Color','k'); 
                ...
                % Legend
                LegendLabel = {'GFP';CompN{n}};
                Lgd = legend(LegendLabel); title(Lgd,'Component','Color','k');
                
                % 3. TOPOPLOTS
                slider_plot(Fig,EEGTEMP,MaxPos,Chanlocs)
                ...
                p=uitable('Parent', f,'Data',to_display,'ColumnEdit',[false true],'ColumnName',...
                    {'Files', 'Peak GFP (in TF)'},'ColumnWidth', {180,100},...
                'CellEditCallBack','MaxGFPList = get(gco,''Data'');');
                p.Position = [10 -100 350 400]; 
                ...
                % Store results
                MaxIndivGFP.(Levels{k})(m) = MaxGFPList{Pos,2}; 
    ...
    if strcmpi(PromptIndivGFP,'semi-automatic')
        TEMP = cell2mat(struct2cell(MaxIndivGFP));
        MaxGFPList = TEMP(:);
        MeanPeakGFP = structfun(@(x) mean(x,2),MaxIndivGFP,'UniformOutput',0);

        % Compute SD over individual GFP peaks for each level
        SDPeakGFP = structfun(@(x) std(x),MaxIndivGFP,'UniformOutput',0);
    end

    % MANUAL INDIVIDUAL GFP PEAKS LOCALISATION (e.g. using CARTOOL)
    if strcmpi(PromptIndivGFP,'Manual')
...
%         TEMPGFPData = MeanGFP.(Fields{m})(TEMPCompinTF(1):TEMPCompinTF(2));
%         MaxGFP.(Fields{m}).Value = round(MeanPeakGFP.(Fields{m}));
%         MaxGFP.(Fields{m}).Pos = find(MeanGFP.(Fields{m}) == MaxGFP.(Fields{m}).Value);
...
%         PosRectinMS = [TEMPComp(1) 0 TEMPComp(2)-TEMPComp(1) max(MeanGFP.(Fields{m}))];
        ...
        % Legend
%         if m==length(Fields)
%             Lgd = legend({'Mean GFP';'MeanGFPPeak +- 1SD'}); 
% %             title(Lgd,'POI','Color','k')
%         end
    ...
    % Save figure 
    FigName = [p2 '\' OutputFolder '\GFPPeak_' CompN{1} '_' PromptIndivGFP '_' date_name '.png'];
