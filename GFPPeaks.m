...
I_p = strfind(p,'\');
p2 = p(1:I_p(end)-1);
date_name = datestr(now,'dd-mm-yy_HHMM'); % time and date
...
PromptIndivGFP = inputdlg({['Extension of the files to be loaded:' newline ...
    ...
    '1) semi-automatic' newline '2) manual' newline '3) Load previous data & parameters']},'Settings',1,{'1','1'});
...
% Only accessed if not loading previous parameters
if ~strcmpi(Method,'load')
    ...
    PromptInstructions = {'Enter the epoching interval (in ms)','Enter the sampling rate:',...
        'Enter the number of EEG channels','Enter the output folder name'};
    PromptValues = {'-100 700','1024','64','Output'};
    ...
    numchannels = str2double(PromptInputs{3});
    OutputFolder = PromptInputs{4}; mkdir([OutputPath '\' OutputFolder]);
    if numchannels>63
    ...
    % Save parameters
    save([OutputPath '\' OutputFolder '\Parameters_' date_name '.mat'],...
        '-regexp', '^(?!(e|eF|eS|eW|f|p)$).')

% Loading the parameters file
else
    [FileName, Folder] = uigetfile('*.mat', 'Select a parameters file');
        ...
        if strcmpi(PromptBlind,'No')
            ...
            for m=1:length(Levels) % For each level of the factor
                ...
                for n=1:length(LevelsFilesList) % For each file
                    ...
                    [~,numtimeframes,samplingrate,thedata]=openeph(Path);
...
%                     ElectToCol = str2double(CompList(~cellfun(@(x) isempty(x),CompList(:,end)),end));
    ...
    TabOut = [Unblinding cell(size(Unblinding,1),6)];
    ...
    for m=1:length(Fields) % For each level
        ...
        MaxGFP.MeanRange = mean(GFPData.(Fields{m})(Low:High,:));
        MaxGFP.SDRange = std(GFPData.(Fields{m})(Low:High,:));    
        ...
        MaxVolt.MeanRange = mean(TEMPDat);
        MaxVolt.SDRange =std(TEMPDat);
        
        % Filling the output table (according to Fields)
        Idx = contains(Unblinding(:,2),Fields{m});
        TabOut(Idx,5:end) = num2cell([MaxGFP.MeanRange' MaxGFP.SDRange' ...
        MaxVolt.MeanRange' MaxVolt.SDRange']);
        
    end
    
    % Saving filenames in order
    MaxGFP.Files = Unblinding;
    ...
    TabOut(:,3:4) = num2cell([MaxGFPList MaxGFPms]);
    TabOut = array2table(TabOut);
